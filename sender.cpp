...
int main(){
    ...
    std::cout << "[W] Accel | [S] Brake | [D] Door Toggle | [Q] Quit" << std::endl;
    while(true){
        int nb =read(s,&frame_rx,sizeof(struct can_frame));
        if (nb > 0) {

        if (frame_rx.can_id == 0x666) {
            rpm = 0; 
